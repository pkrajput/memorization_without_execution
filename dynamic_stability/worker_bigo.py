#!/usr/bin/env python3
"""
Disposable Worker for Fail-safe Evaluator
-------------------
This script is executed as a subprocess by the main `produce_dynamic_traces_bigo.py` watchdog.
"""
...
# --------------------------------------------------------------------------- #
# Low‑level worker (runs inside a thread) - CORRECTED LOGIC
# --------------------------------------------------------------------------- #
def _inner_test_worker(
    ...
    try:
        globs: Dict[str, Any] = {}
        # Use the StdioShim to capture stdout from print() calls
...
if __name__ == "__main__":
    try:
        ...
        # Exit with a non-zero code to signal failure to the watchdog
        sys.exit(1)
